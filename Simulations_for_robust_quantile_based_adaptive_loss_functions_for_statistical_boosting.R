...
# M_Bisquare
M_Bisquare <- function (tau = 0.99 , k=NULL, pi=NULL){ 
  k_to_tau <- function(k){
...
M_Quant_Huber <- function(tau= 0.8213748 , k=NULL , pi=NULL , q = 0.5){
  k_to_tau <- function(k){
    return((pnorm(k)-pnorm(-k))) # =1-2*pnorm(-k)
  }
...
Simfunc <- function(id,p=10,n=200){
  ...
  for (i in 1:length(corrupted2)) { # loop for different amount of corruption in the data
    ...
    boost_Huber             <- glmboost(y~.,family=M_Quant_Huber(tau=tau_huber),control = boost_control(mstop=1),data=datasimulation)
    ...
    boost_bisquare        <- glmboost(y~.,family=M_Bisquare(tau=tau_bisquare),control = boost_control(mstop=1),data=datasimulation)
